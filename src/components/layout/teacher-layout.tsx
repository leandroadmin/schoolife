...
import { Users2, LayoutDashboard, LogOut, PanelLeftClose, PanelLeftOpen, Loader2, CalendarDays, MessageSquare, ClipboardCheck, FileText } from "lucide-react"
...
export function TeacherLayout() {
    ...
    const navigationLinks = [
        ...
        { path: "/teacher/attendance", icon: ClipboardCheck, label: "Frequência" },
        { path: "/teacher/evaluations", icon: FileText, label: "Avaliações" },
        { path: "/teacher/announcements", icon: MessageSquare, label: "Comunicados" },
        { path: "/teacher/calendar", icon: CalendarDays, label: "Calendário" },
    ]
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans selection:bg-amber-500/30">
            {/* Sidebar */}
            <aside
                className={cn(
                ...
                <div className="h-20 flex items-center px-6 border-b border-slate-100 dark:border-slate-800 shrink-0">
                    <div className="flex items-center gap-3 w-full">
                        {settings?.logo_url ? (
                            ...
                            <>
                                <div className="min-w-10 h-10 rounded-[14px] bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/20">
                                ...
                                <div className={cn("flex flex-col overflow-hidden transition-opacity duration-300", !isSidebarOpen && "md:opacity-0 md:w-0")}>
                                    ...
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mt-1">do Professor</span>
                                </div>
                            </>
                        )}
                    </div>
                ...
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
                    <div className={cn("flex items-center gap-3 px-2 mb-4 transition-all duration-300", !isSidebarOpen && "md:justify-center")}>
                        ...
                        <div className={cn("flex flex-col overflow-hidden", !isSidebarOpen && "md:hidden")}>
                            <span className="text-sm font-bold text-slate-900 dark:text-white truncate">{teacherName}</span>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Professor</span>
                        </div>
                    </div>
            ...
            <main className="flex-1 flex flex-col h-full relative overflow-hidden bg-slate-50 dark:bg-slate-950">
                ...
                <header className="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 flex items-center justify-between px-6 shrink-0 z-10 sticky top-0">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-lg font-black tracking-tight text-slate-800 dark:text-white hidden sm:block">Painel de Controle</h1>
                    </div>
                </header>
